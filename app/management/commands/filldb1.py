...
class Command(BaseCommand):
    ...
    def fill_likes_on_question(self, n):
        ...
        likes = []
        for i in range(n):
            like = VoteForQuestion.objects.create(question_id=choice(questions), author_id=choice(users),
    ...
    def fill_likes_on_answer(self, n):
        ...
        likes = []
        for i in range(n):
            like = VoteForAnswer(answer_id=choice(answers), user_id=choice(users),
                                 vote=faker.random.randint(-1, 1))
            likes.append(like)
